...
markers = ['s']
figsfolder = f'results/{corpus}/{LLM}/figs/model_validation/Nbits{Nbits}/'
...
alphamin_list = [1E-1]#,1E-4,1E-3,1E-2,1E-1]
...
for sub_length_id,sub_length in enumerate(sub_lengths):
  ...
  for layer_aux_id,layer_id in enumerate(layers):
    ...
    figh.savefig(figsfolder + f'sublength_{sub_length}_l{layer_id}_alphamin{B.alphamin:.5f}_alphamax{B.alphamax:.5f}.pdf',bbox_inches='tight')      
    plt.close()
