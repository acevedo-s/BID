...
class Optimizer():
  ...
  def __init__(self,
              key=0,
              d0=0.,                    # BID
              d0_r=0.,                  # BID + random perturbation
              d1=0.,                    # slope
              d1_r=0.,                  # slope + random pertubation
              delta=0.,                 # step_size
...
def check_initial_condition(Op):
  """ 
  Before doing the optimization, 
  let's save some time checking if the initial conditions fail
  """
  Op.d0_r = Op.d0
  Op.d1_r = Op.d1
  Op = compute_Pmodel(Op)
  nan = jnp.size(jnp.nonzero(jnp.isnan(Op.Pmodel))) # this should be 0
  Op = compute_KLd(Op)
  inf = jnp.size(jnp.nonzero(jnp.isinf(Op.KL))) # this should be 0
  return nan,inf
