  ...
  for key_id,key in enumerate(class_list):
    ...
    for layer_id,layer_name in enumerate(layer_names):
      ...
      if len(minKL_id)==0:
        ...
        minKL_id = minKL_id[0]
      sigma_list[key_id,layer_id,crop_id] = sigmas[minKL_id] / norm
      alpha_list[key_id,layer_id,crop_id] = alphas[minKL_id]
      KL_list[key_id,layer_id,crop_id] = logKLs[minKL_id]
      if fail:
        print(f'{key},{layer_name}: {sigmas[minKL_id]=}')
...
figa.savefig(figsfolder + 'alpha_crop.pdf',bbox_inches='tight')
...
plt.tight_layout()
figs.savefig(figsfolder + f'sigma_crop_n{normalize}_log{log_scale}.pdf',
            bbox_inches='tight'
             )
...
figKL.savefig(figsfolder + 'KL_crop.pdf',bbox_inches='tight')
