...
d0 = sigmas[:,0]/N_list
x0 = N_list.astype(float)
ax0.scatter(np.log(x0),
            np.log(d0),
            )
(a,b),cov = np.polyfit(np.log(x0),np.log(d0),1,cov=True)
print(f'{a=}')
print(f'{b=}')
print(f'{cov=}')
ax0.plot(np.log(N_list),
         a * np.log(N_list) + b ,
         '-',
         color='black',
         label=f'slope={a:.2f} +- {cov[0][0]:.2f}'
         )
...
#   ax0.set_ylim(0,.4)
if False:
  (a,b,gamma),cov = curve_fit(scaling,
                        x0,
                        d0,
                        maxfev=10000,
                        p0=(6,0.1,1.),
                        bounds=([0,0,0],[100,1,5])
                        )
  print(f'{a=}')
  print(f'{b=}')
  print(f'{gamma=}')
  ...
  ax0.plot(x0,
          scaling(x0,a,b,gamma),
          color='black',
          # label=f'offset={b:.2f} +- {cov[1][1]:.3f}' 
          )
# ax0.set_ylabel('BID(T=0)/N')
# ax0.set_xlabel('N')
ax0.set_ylabel('log(BID(T=0)/N)')
ax0.set_xlabel('log(N)')
ax0.legend()
fig0.savefig(figsfolder + f'd_00.pdf',
             bbox_inches='tight',
             )   